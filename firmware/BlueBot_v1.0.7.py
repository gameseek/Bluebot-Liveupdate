...
DRIVE_RAMP_PER_SEC = 4.0
GYRO_TURN_MAX_DPS = 180.0
GYRO_STEER_KP = 0.35
GYRO_Z_SIGN = 1.0
    ...
    write_settings(
        {
            ...
            "led_animation": int(led_animation),
            "led_speed": int(led_speed)
        }
    )
...
def update_drive_control():
    ...
    if (
        ...
        and
        (
            abs(mpu_pitch) > TILT_STOP_DEG
            or
            abs(mpu_roll) > TILT_STOP_DEG
        )
    ):
...
while True:
    ...
    # MPU is also the local motor feedback sensor.
    # Read it every main-loop cycle.
    read_mpu()


    # Run motor controller continuously.
