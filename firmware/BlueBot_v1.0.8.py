...
# Local sensors used for safety
SENSOR_INTERVAL_MS = 40
...
# Prevent stale joystick command
DRIVE_COMMAND_TIMEOUT_MS = 500

# Delay before saving a changed speed to flash
...
# Approximate 10 cm reverse.
# Adjust this value after physical testing.
CLIFF_BACKOFF_MS = 500

# 30% reverse power
CLIFF_BACKOFF_POWER = 0.30

# Pause after backing away
CLIFF_HOLD_MS = 250
...
# Smoother acceleration than v1.0.7
...
# Prevent MPU correction becoming too aggressive
...
def cliff_detected():
    ...
    return floor_cliff == 1


def update_sensor_multiplexer():
    ...
    if sharp_instant_cm >= 0:
        ...
        if not front_obstacle:
            ...
            if sharp_instant_cm <= FRONT_STOP_CM:

                front_obstacle = True

        else:

            if sharp_instant_cm >= FRONT_RELEASE_CM:

                front_obstacle = False


    # --------------------------------------------------------
    # CLIFF HAS HIGHEST PRIORITY
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
        ...
        if time.ticks_diff(
            ...
            print(
                "SAFETY: BACKING FROM CLIFF"
            )


    # --------------------------------------------------------
    # BACK AWAY APPROXIMATELY 10 CM
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
        ...
        if time.ticks_diff(
            ...
            # Do not drive again until floor
            # is confirmed under sensor.
...
def update_drive_control():
    ...
    if (
        ...
        target_input_left = 0.0
        target_input_right = 0.0
        ...
        # Equal reverse command.
        # MPU will keep this movement straight.
        ...
        target_input_left = requested_left
        target_input_right = requested_right


        # ====================================================
        # FRONT OBJECT PROTECTION
        # ====================================================
        ...
        if front_obstacle:
            ...
            # Positive throttle means forward.
            # Reverse and rotation remain available
            # so the operator can escape.
    ...
    # ========================================================
    # CONVERT LEFT/RIGHT TO THROTTLE/TURN
    # ========================================================
    ...
    if mpu_available:
        ...
        # During automatic cliff reverse,
        # use the backoff power as the speed scale.
    ...
    if (
        ...
        # Backoff power is already limited above.
        # Do not multiply it by user speed again.
    ...
    target_left *= speed_factor

    target_right *= speed_factor
    ...
    if target_left > drive_left_output:
    ...
    if target_right > drive_right_output:
    ...
    actual_left = drive_left_output

    actual_right = drive_right_output


    # ========================================================
    # EXISTING PHYSICAL MOTOR CORRECTION
    # ========================================================
...
def update_led_animation():
    ...
    # Safety always overrides normal LED mode.
...
def read_sharp():
    ...
    try:
        ...
        # Fast unfiltered distance is used
        ...
        # Filtered value remains for display/app.
...
def read_tof():
    ...
    if (
        ...
        # Invalid / no return is treated
        # as unsafe for cliff protection.
...
def update_fast_sensors():

    read_sharp()

    read_tof()

    update_sensor_multiplexer()
...
def process_command(command):
    ...
    if command.startswith(
        ...
        try:
            ...
            # Do NOT write flash for every slider update.
            # Save later after slider settles.
