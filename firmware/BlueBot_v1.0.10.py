...
# MPU telemetry only.
# MPU is read much faster locally.
...
# v1.0.9 = 500 ms
# Increased because the app may not continuously resend
# an unchanged joystick position.
DRIVE_COMMAND_TIMEOUT_MS = 3000
...
# Absolute minimum stop distance
FRONT_HARD_STOP_CM = 15.0

# High-speed detection distance
FRONT_MAX_BRAKE_CM = 40.0

# Minimum distance needed before wall condition clears
FRONT_RELEASE_MIN_CM = 20.0

# Additional clearance after dynamic stop point
FRONT_RELEASE_MARGIN_CM = 3.0

# v1.0.9 used 2.
# Safety now reacts on first dangerous measurement.
...
# Active brake before reversing
CLIFF_BRAKE_MS = 120

# Approximate 10 cm reverse.
# Tune experimentally because there are no wheel encoders.
...
def start_emergency_brake():
    ...
    # Do not continually restart the brake timer
    # if safety code calls this again.
...
def update_emergency_brake():
    ...
    if time.ticks_diff(
        ...
        # Release short brake.
        # Motors now coast with zero PWM.

        release_motor_output()


# ============================================================
# IMMEDIATE MOTOR OFF WITHOUT BRAKE
# ============================================================
...
def stop_motors():
    ...
    # Use a short active brake even for STOP.
    # It will automatically release after 100 ms.
...
# ============================================================
# DYNAMIC FRONT STOP DISTANCE
# ============================================================

def get_front_stop_distance():

    # actual_left/right represent the current
    # logical motor output after speed limiting.
...
def update_front_safety():
    ...
    if not front_obstacle:
        ...
        if (
            ...
            # Remove the old forward request.
...
def update_cliff_safety():
    ...
    if (
        ...
        if time.ticks_diff(
            ...
            # Brake again when the ~10 cm
            # reverse movement is complete.
...
def update_drive_control():
    ...
    if (
        ...
        # Motor remains zero after active brake releases.
        ...
        if front_obstacle:
            ...
            # Forward is prohibited while wall
    ...
    if (
        ...
        # Backoff power is already fixed.
        speed_factor = 1.0
...
def read_sharp():
    ...
    try:
        ...
        # Fast averaging for the safety sensor.
        ...
        # Immediate reading for collision safety.
...
def process_command(command):
    ...
    if command.startswith(
        ...
        try:
            ...
            # Do not write flash continuously.
